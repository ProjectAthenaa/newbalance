...
func (tk *Task) ATC() {
	req, err := tk.NewRequest("POST", "https://www.newbalance.com/on/demandware.store/Sites-NBUS-Site/en_US/Cart-AddProduct?__Analytics=athena", []byte(fmt.Sprintf("pid=%s&quantity=1&estimatedDelivery=Estimated+delivery+-+2-5+business+days+once+shipped&options=%%5B%%5D", tk.VariantId)))
	if err != nil {
